...
export function Input({ label, error, hint, rightElement, className, ...props }: InputProps) {
  return (
    <label className="flex w-full flex-col gap-2 text-sm text-slate-200">
      ...
      <span
        className={cn(
          'relative flex h-12 items-center rounded-2xl border bg-slate-950/70 px-4 transition',
          error
            ? 'border-rose-400/60 focus-within:border-rose-300'
            : 'border-white/10 focus-within:border-cyan-300/60 focus-within:bg-slate-950/90',
        )}
      >
        <input
          className={cn(
          ...
          {...props}
        />
        {rightElement ? <span className="absolute right-4 text-slate-400">{rightElement}</span> : null}
      </span>
