...
export function parseTelnyxWebhook(rawJson: unknown): NormalizedTelnyxEvent {
  ...
  const gatherStatus = getString(payload.status) || (gatherResult ? getString(gatherResult.status) : '');
  const messageHistory = pickArray(payload, ['message_history']);
...
export function extractGatherResult(event: NormalizedTelnyxEvent): GatherExtraction {
  if (event.eventType !== 'call.ai_gather.ended') {
    throw new MissingGatherPayloadError('Gather extraction is only valid for call.ai_gather.ended.');
  }
